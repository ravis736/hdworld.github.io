...
if ( !defined('NONCE_KEY') ) die('Die!');
    ...
    $term = get_term( $term_id, 'server' );

    if(isset($_POST['submit_edit_server'])){
        ...
        if(empty($error)){
            ...
            wp_update_term($term->term_id, 'server', array(
            ...
            if(!empty($_FILES['poster']['tmp_name'])){
                ...
                move_uploaded_file($_FILES["poster"]["tmp_name"], $upload_dir['path'].'/server-'.$term->term_id.$ext);

                $filetype=wp_check_filetype($_FILES["poster"]["name"]);

                $attachment = array(
                    'guid' => $upload_dir['url'].'/server-'.$term->term_id.$ext, 
                ...
                $attach_id = wp_insert_attachment( $attachment, $upload_dir['path'].'/server-'.$term->term_id.$ext, '');

                require_once(ABSPATH . 'wp-admin/includes/image.php');
                
                $attach_data = wp_generate_attachment_metadata( $attach_id, $upload_dir['path'].'/server-'.$term->term_id.$ext);
            ...
            $ok='<p class="msjadm-ok">'.__('The server was updated successfully.', TRMOVIES).'</p>';
...
?>
<main>
    <form action="admin.php?page=tr-movies-movie&action=links&action2=servers" method="post" class="Blkcn" enctype="multipart/form-data">
        <p class="Title"><?php _e('Edit Server', TRMOVIES); ?> <span>(<a target="_blank" href="term.php?taxonomy=server&tag_ID=<?php echo $term->term_id; ?>"><?php _e('Edit term', TRMOVIES); ?></a>)</span></p>
        ...
        <p><button class="BtnSnd BtnStylA BtnFlR" name="submit_edit_server" type="submit"><?php _e('Save changes', TRMOVIES); ?></button></p>
