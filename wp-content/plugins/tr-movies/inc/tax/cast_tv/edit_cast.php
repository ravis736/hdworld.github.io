...
if ( !defined('NONCE_KEY') ) die('Die!');
    ...
    if(isset($_POST['submit_edit_cast'])){
        ...
        if(empty($error)){
            ...
            wp_update_term($term->term_id, 'cast_tv', array(
...
?>
<main>
    <form action="admin.php?page=tr-movies-tv&action=cast" method="post" class="Blkcn" enctype="multipart/form-data">
        <p class="Title"><?php _e('Edit Actor', TRMOVIES); ?> <span>(<a target="_blank" href="term.php?taxonomy=cast_tv&tag_ID=<?php echo $term->term_id; ?>"><?php _e('Edit term', TRMOVIES); ?></a>)</span></p>
        ...
        <p class="InpFlImg img_actor">
            ...
            <?php }elseif($image=='' and $image_hotlink!=''){ ?>
