...
<section>
    ...
    <div class="Top">
        <h1><?php _e('Categories', TRMOVIES); ?></h1>
        <?php tr_movies_menu(); ?>
    </div>
    ...
    <div class="AdmCls AltClBx">
        <?php
            if(empty($_GET['edit']) and empty($_POST['edit'])){
                include( plugin_dir_path( __FILE__ ) . '/add_category.php');
            }else{
                include( plugin_dir_path( __FILE__ ) . '/edit_category.php');                
            }
        ?>
        <?php include( plugin_dir_path( __FILE__ ) . '/del_category.php'); ?>
        <aside>
            <?php echo $error.$ok; ?>
            <form action="<?php echo admin_url('admin.php?page=tr-movies-movie&action=categories'); ?>" method="post">
                <div class="TPTblCn TblCnAdm">
                    <?php
                        ...
                        $number = count(get_terms('category', array( 'hide_empty' => false )));
                                ...
                                foreach ($terms as $term) {
                                    echo '
                                        <tr>
                                            ...
                                            <td class="td1ln"><a class="edtlnk" href="admin.php?page=tr-movies-movie&action=categories&edit='.$term->term_id.'"><i class="dashicons dashicons-edit"></i>'.__('Edit', TRMOVIES).'</a><a onclick="return confirm(\''.__('Are you sure?', TRMOVIES).'\');" class="dltlnk" href="admin.php?page=tr-movies-movie&action=categories&del='.$term->term_id.'"><i class="dashicons dashicons-trash"></i>'.__('Delete', TRMOVIES).'</a></td>
                                        </tr>
                                    ';
                                }                  
                            ?>
                    ...
                    <?php
                            ...
                            _e('Add your first category :)', TRMOVIES);
                        }
                    ?>
                </div>
                <?php
