...
if ( !defined('NONCE_KEY') ) die('Die!');
    ...
    if(isset($_GET['del'])){
        ...
        wp_delete_term( intval($_GET['del']), 'category' );
        
        echo '
            <script type="text/javascript">
                window.location = "'.admin_url('admin.php?page=tr-movies-movie&action=categories&delok=1').'";
            </script>
        ';
    ...
    if(isset($_POST['delmult'])){
        ...
        if(empty($error)){
            ...
            foreach ($_POST['del'] as &$value) {
                wp_delete_term( intval($value), 'category' );
            }
