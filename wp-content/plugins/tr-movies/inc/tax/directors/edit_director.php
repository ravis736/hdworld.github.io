...
if ( !defined('NONCE_KEY') ) die('Die!');
    ...
    $term = get_term( $term_id, 'directors' );
    ...
    if(isset($_POST['submit_edit_director'])){
        ...
        if(empty($_POST['name'])){
            ...
            $parent_term = term_exists( stripslashes( wp_strip_all_tags( $_POST['name'] ) ), 'directors' );
        ...
        if(empty($error)){
            ...
            wp_update_term($term->term_id, 'directors', array(
...
?>
<main>
    <form action="admin.php?page=tr-movies-movie&action=directors" method="post" class="Blkcn">
        <p class="Title"><?php _e('Edit Director', TRMOVIES); ?> <span>(<a target="_blank" href="term.php?taxonomy=directors&tag_ID=<?php echo $term->term_id; ?>"><?php _e('Edit term', TRMOVIES); ?></a>)</span></p>
