...
jQuery(document).ready(function($){
    ...
    $('.Select-Md').searchableOptionList({
        showSelectAll: false,
        texts: {
            noItemsAvailable: trsol.error,
            quickDelete: '&#xE5CD;',
            searchplaceholder: trsol.placeholder
        }
    });
    ...
    function tp_suggets(strclass, strarray){
        ...
        $('body').delegate( '.'+strclass, 'keyup', function(event){ 
            ...
            if(value.length==0){result.next().hide();}
            if(value.length>=3){
        ...
        $(document.body).on('click', '.trsrcbx button' ,function(event){
            ...
            if (!$( '#'+nameinput+'-'+id+'' ).length ) {
                $(this).parent().parent().parent().parent().find('ul').append('<li id="'+nameinput+'-'+id+'"><span class="tr-selected">'+name+'</span><span class="tr-delete">&#xE5CD;</span><input name="'+nameinput+'[]" type="hidden" value="'+id+'"></li>');
            }
            $('.tpsuggets').val('');
            $('.trsrcbx div').html('')
            $('.trsrcbx').hide();
    ...
    tp_suggets('tpsuggets_casts', tr_arr_casts);
    tp_suggets('tpsuggets_countries', tr_arr_countries);
    tp_suggets('tpsuggets_directors', tr_arr_directors);
